...
class Trainer(object):
    ...
    def setup(self):
        args = self.args
        sub_dir = 'VGG/NWPU/1-14-input-{}-lr-{}-Count'.format(args.crop_size, args.lr)
        ...
        if args.dataset.lower() == 'qnrf':
            self.datasets = {'train': Crowd_qnrf(os.path.join(args.data_dir, 'train_data'),
                                               args.crop_size, downsample_ratio, 'train'),
                             'val': Crowd_qnrf(os.path.join(args.data_dir, 'test_data'),
    ...
    def val_epoch(self):
        ...
        for inputs, count, name in tqdm.tqdm(self.dataloaders['val']):
            with torch.no_grad():
                inputs = tensor_divideByfactor(inputs, factor=8)
                inputs = inputs.to(self.device)
