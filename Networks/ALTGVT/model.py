...
class Reg(nn.Module):
    def __init__(self):
        ...
        self.conv_last = nn.Sequential(nn.Conv2d(128, 64, 3, padding=1, dilation=1),
                                       nn.BatchNorm2d(64),
                                       nn.ReLU(inplace=True),
                                       nn.Conv2d(64, 64, 3, padding=2, dilation=2),
                                       nn.BatchNorm2d(64),
...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=1, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        for i in range(len(depths)):
            if i == 0:
                ...
                self.patch_embeds.append(
                    PatchEmbed(img_size, 1, embed_dims[i - 1], embed_dims[i]))
...
class CPVTV2(PyramidVisionTransformer):
    ...
    def forward(self, x):
        ...
        #print('2', x[1].size(), x[2].size(), x[3].size())
        x = self.regression(torch.cat((x1, x), dim=1))

        return x


class PCPVT(CPVTV2):
    def __init__(self, img_size=224, patch_size=1, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256],
...
class ALTGVT(PCPVT):
    ...
    alias Twins-SVT
    """
    def __init__(self, img_size=224, patch_size=1, in_chans=64, num_classes=1000, embed_dims=[64, 128, 256],
                 num_heads=[1, 2, 4], mlp_ratios=[4, 4, 4], qkv_bias=False, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
...
def alt_gvt_large(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        checkpoint = torch.load('/home/xuzhiwen/home/xuzhiwen/dataset_wfs/TransDensityNet/Networks/alt_gvt_large.pth')
        model.load_state_dict(checkpoint, strict=False)
        print("load transformer pretrained")
    return model

@register_model
def alt_gvt_large_weak(**kwargs):
    model = ALTGVT(
        patch_size=1, embed_dims=[64, 64, 64, 64], num_heads=[4, 4, 4, 4], mlp_ratios=[4, 4, 4, 4],
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[4, 4, 4, 4], wss=[8, 8, 8, 8], sr_ratios=[8, 4, 2, 1],
        **kwargs)
    model.default_cfg = _cfg()
    return model
