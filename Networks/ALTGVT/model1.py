...
class ALTGVT(PCPVT):
    ...
    alias Twins-SVT
    """
    def __init__(self, img_size=224, patch_size=4, in_chans=64, num_classes=1000, embed_dims=[64, 128, 256],
                 num_heads=[1, 2, 4], mlp_ratios=[4, 4, 4], qkv_bias=False, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
...
@register_model
def alt_gvt_large_weak(pretrained=False, **kwargs):
    model = ALTGVT(
        patch_size=4, embed_dims=[64, 64, 64, 64], num_heads=[4, 4, 4, 4], mlp_ratios=[4, 4, 4, 4],
        qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[4, 4, 4, 4], wss=[8, 8, 8, 8], sr_ratios=[8, 4, 2, 1],
        **kwargs)
    model.default_cfg = _cfg()
    return model
