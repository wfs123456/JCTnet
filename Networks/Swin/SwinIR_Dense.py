...
class RSTB(nn.Module):
    ...
    def forward(self, x, x_size):
        return self.patch_embed(self.conv(self.patch_unembed(self.residual_group(x, x_size), x_size)))
...
class SwinIR(nn.Module):
    ...
    def forward_features(self, x):
        ...
        #     x = layer(x, x_size)
        x0 = x
        x = self.layers[0](x, x_size) + x0
        x1 = x
        x = self.layers[1](x, x_size) + x0 + x1
        x2 = x
        x = self.layers[2](x, x_size) + x0 + x1 + x2
        x3 = x
        x = self.layers[3](x, x_size) + x0 + x1 + x2 + x3
...
def Net(**kwargs):
    model = SwinIR(img_size=(256, 256), window_size=4, img_range=1., depths=[8, 8, 8, 8],
